...
func AuthSocks5(conn network.Connection) error {
	...
	if socksVersion != 5 {
		logrus.Fatal("Socks version should be 5, now is", socksVersion)
	}
	...
	if !common.In(byte(0), authWays) {
		logrus.Warn("Only support [NO AUTHENTICATION REQUIRED] auth way.")
	}
...
func ConnectRemote(conn network.Connection, remoteServer string, password string) (network.Connection, error) {
	...
	if socksVersion != 5 {
		logrus.Warn("Socks version should be 5, now is", socksVersion)
	}
	...
	// 仅支持通过TCP协议进行流量转发
	if command != 1 {
		logrus.Warn("Only support [CONNECT] command")
	}
	...
	default:
		logrus.Fatal("Not support address type", addrType)
	}
